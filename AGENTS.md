...
- Settle what a review found with `triage-findings`, and keep the Disposition Record it produces
  ...
  Round ranges do not tile, and what carries the chain instead is the `Prior round` field.
  The round chain section at the end of this one states both, and takes the field's rule
  from the check that reads it.
...
Round ranges do not tile, and are not meant to. A range is a base and the head it reviewed, where
the base is whatever that round started from — the release tag through one stretch of this history,
the previous settle commit through another — so consecutive ranges overlap, and a commit lying
outside all of them is not by itself a hole in the ledger.

The field is written down because it drifted and nothing caught it. One round's `Prior round` named
a record other than its neighbour, and every id the rounds it skipped had left open lost its
lifecycle home: no later record mentions them, and the round that skipped them declared an older
record's open set out of scope in their place. Measured over every record kept here, no other field
departs from the rule below — which is what makes it a rule the corpus already keeps, rather than a
shape imposed on it afterwards.

The rule is generated from `scripts/round_chain.py`, which is also what enforces it, so the two
cannot disagree. Regenerate with `.venv/bin/python scripts/round_chain.py --write`; the gate's round
chain step fails while the block is stale. Writing it here by hand is what produced the drift this
section records: the clauses stood in this guide, in a docstring and in a settled record's
correction at once, and disagreed on the day they landed.

<!-- ROUND-CHAIN-RULE:START (generated by scripts/round_chain.py — do not edit by hand) -->

- A round record is named for the range it settled, and `Prior round` names the record whose reviewed head immediately precedes this one's.
- A second reading — the `second-reading` suffix — settles the range it re-reads in the same turn, so it sits outside the chain and its own field is not read.
- Either record of a twice-read round identifies that round, so a later round may name either of them, and which records settle one round is decided by resolving their halves rather than by comparing what was written.
- A record name is read whole: an undeclared suffix is reported rather than parsed, and a head `main` does not hold is reported apart from one nothing resolves.

<!-- ROUND-CHAIN-RULE:END -->
...
- **Do not cite a line number in durable prose.** A `path:line` citation is keyed to a coordinate
  ...
  quietly: a review round found wrong symbols and wrong lines inside the cells asserting closures had
