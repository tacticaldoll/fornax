#!/usr/bin/env python3
"""Own the one predicate four types in this directory had each written by hand.

A reader here answers with what it read or with why it could not, never both and never
neither, and the types carrying that pair all guard it the same way. Four of them had
written the guard out: `skill_yaml.Document`, `path_boundary.Boundary`,
`check_citations.Symbols` and `evidence_currency.Fingerprint`.

The shared predicate exists because these four types independently need the same paired-state
invariant; keeping it here avoids each implementation drifting apart.

So what is owned here is the predicate and nothing else, because the predicate is the only
thing the four share. Their accessors do not: `skill_yaml.Document.require` raises its
...
Routed through it: the four types above.
