...
plugin cache carried an earlier release without it, so a subagent anywhere on the machine could not
discover it through the plugin system. Since the later deployment the cache carries the skill, and a
control on this machine is no longer reachable by any arrangement of working directories. The controlled numbers
