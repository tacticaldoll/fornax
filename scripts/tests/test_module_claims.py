...
#:
#: This is the third answer in this module to "which files does this workspace hold", and
#: it stays outside the owner `workspace_test_modules` names on purpose. Their subjects
#: differ: that one walks the whole repository, where `.venv` is what git keeps out
#: without a skip list going stale beside `.gitignore`; this one walks `scripts/` alone,
#: which holds no such tree and no `.py` under `__pycache__`, so the filesystem needs no
#: list either. The boundary is also the stricter subject read from disk — a module a
#: contributor has written and not yet committed can cross it, and should be reported
#: before the commit rather than after. Recorded rather than routed, which is the choice
#: `AGENTS.md` leaves open where an owner does not answer every caller's question.
...
def imports_at(path: Path) -> set[str]:
    ...
    Two import forms are read: a plain `import x` and an absolute `from x import y`. A
    relative one is not, and the enumeration says so rather than leaving a form out of a
    list that reads as complete — the shape a finding in this repository is already named
    for. Nothing is lost by it here: `scripts/` is not a package, so a relative import has
    no parent to climb to and cannot leave the package it sits in. Should that ever change,
    this is the line that stops being true, and it says so from here rather than from a
    round that has to rediscover it."""
...
def entry_point_out_of_place(source: str) -> bool:
    ...
    would a later round running the test a dated guard entry names. The measurement
    that found it is retained as historical evidence.
