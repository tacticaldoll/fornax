...
**Input**: the mature conversation knowledge to persist — a named insight, a prior `assess-knowledge` Knowledge Assessment Record, or a target path to validate; if the source is unclear, ask for the excerpt or intended future use. Resolution detail in Phase 1.
...
| Neither | The assessment found the topic has conversation value but no clear artifact value. Decline, and say which claim would have to hold for it to have a durable home |
