...
**Input**: the conflicting rules, requirements, or the nature of the deadlock — quoted inline, in governance or spec files at a given path, or described in the conversation; if none is given, ask which constraints are in conflict and what they are blocking, and ask for the primary business goal if the context is purely technical.
