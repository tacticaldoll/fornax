...
**Input**: a review findings list — a `static-review` Review Record, findings pasted inline, or a path to either — plus a prior Disposition Record when the user supplies one, and the codebase the findings concern; if none is given, ask which findings to triage.
...
- When the user supplies a prior Disposition Record, every finding it declined or deferred **keeps that
...
Not `file:line`, for the reason Phase 1 gives about keying a finding: a line number drifts with the
next edit to that file, so a Reach keyed to one cannot be checked once the record is a round old,
and a record's Reach is read by every round after it. A symbol survives an unrelated edit and can be
verified against the file; a line number can only be maintained by hand, and by the time it is wrong
nothing says so.
...
| Non-finding sections | the sections the `Findings` field excludes from the count | the row-bearing sections the record contains, whether each declares its finding status, and the section's rows against the gate findings — a row stating what a gate declares a finding, or a finding naming a unit the section omits, surfaces here | pass \| mismatch \| not claimed |
...
|---|---|---|---|---|---|---|
| 1 | stated as the change, not the symptom | every source id | 1a | listed Kind \| `<action> (gap)` | `file` + `symbol`, … | where it goes |
| 1 | " | " | 1b | the alternative, when the cause admits one | `file` + `symbol`, … | " |
