...
- **A record contract exists only where one skill's output is another skill's input.** Most skills
  ...
  `handle-feedback` and `static-review` → `triage-findings`, `docs/record-contracts.md`). The
...
- **Release identity is resolved, not copied.** A release check reads the version tag from the
  canonical remote and records the resolved commit in the check's scope. Repository prose does not
  maintain a standing tag-to-SHA mapping; dated evidence records a replacement only when one is
  observed.
