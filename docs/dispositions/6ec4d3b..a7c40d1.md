...
| 4 | The cell escape is tested by a lookbehind rather than consumed, so a backslash that is itself escaped reads as escaping the delimiter after it | ESCAPE-TESTED-NOT-CONSUMED | 4a consume escapes left to right when splitting a row | `guard` | `record_shape.cells` | `plan-implementation` |
...
| 7 | Two expressions do more than their purpose — a comprehension that copies a list unchanged, and a sort taken only for a length | NO-OP-COMPREHENSION, SORT-TO-COUNT | 7a write each as what it does | `restate` | `record_shape.cells`, `record_shape.main` | `plan-implementation` |
