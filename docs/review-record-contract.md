...
**Status**: deterministic fixtures cover the current record shape. The fresh-context behavioral
evidence recorded in `scripts/tests/scenarios/triage-findings/README.md` predates the current wording;
read its dated results and release blockers separately from this current field inventory.
...
Generated from the two skills by `scripts/seam_contract.py`; `--check` fails when it no longer
matches them. The seam list is derived from matching optional `skill-interface.yaml` declarations
rather than prose, so a second seam appears without an edit and zero seams is a clean answer.
