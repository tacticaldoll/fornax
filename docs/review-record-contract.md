...
matches them. The seam list is derived from the consumer's `**Input**:` line rather than maintained
here, so a second seam appears without an edit and zero seams is a clean answer.
...
### `static-review` → `triage-findings` — Review Record
