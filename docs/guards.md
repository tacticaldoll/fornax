...
| DEFAULTS-ON-THE-INNER-CHECKS | for 4a, the removal of the default from the three inner checks | `test_validate_skills.FillingAskedNotDefaultedTests.test_an_inner_check_will_not_supply_a_filling_of_its_own` — it calls the manifest check without a filling and asserts it refuses. The signal is weak by construction: the caller passes positionally either way, so what the case holds is the refusal and not any difference in what the validator answers | **measured** — restoring the default on the manifest check turns 1 red |
| DEFINITION-NAMED-ON-THE-BINDING, ALIAS-REASON-OVERREACHES | — | none by test. The units are sentences in `skill_model`'s module docstring, and no gate opens over a docstring's claim about which name owns a value. 1b's half is different: removing the `STATUSES` binding is caught by the citation gate step wherever a record still names it as a symbol, which is the only signal a removal has here | **measured** — the removal was refused by the gate until this row and the settlement's Reach cell stopped naming it as a live symbol; the pre-commit hook reported two documents. The citation form in both was changed, not the reading |
...
| Finding | Revert this, once its repair lands | Guard | Red on revert |
|---|---|---|---|
| PACKAGE-EDGE-OUT-AND-BACK | for 1d, the assertion that no package module imports outside the package | that assertion itself, which is the only mechanism the boundary has ever had. Whichever of 1a, 1b or 1c lands, reverting it turns the assertion red; without 1d, reverting any of them turns nothing red, which is how the edge reached two commit messages as a verified claim | **measured** — restoring the `TYPE_CHECKING` import turns 1 red, taken here and again in a later round's isolated clone. The row said no repair had landed while 1b and 1d were the first commit of the range it was written in; a prospective cell holds no reading, so filling it in completes the row rather than correcting one |
| MAPPING-NOT-PROXIED-BY-TYPE | for 2a, the wrap in `FormatSchema.__post_init__` | `test_validate_skills.SchemaImmutabilityTests.test_a_schema_built_by_replace_is_no_more_writable_than_the_declared_one` — it builds a schema through `dataclasses.replace` from a plain mapping and asserts the result refuses a write. The existing mapping case cannot carry it: it reads the one value that was wrapped by hand, which is why the residue survived the repair that closed the finding above it | **measured** — removing the wrap turns 2 red: this case and the declared filling's, which the wrap now supplies. It read 3 before the mapping case gained a cleanup; the third was a family that case left behind for the next one in string order, and a count that moves with a rename is not the guard's own |
...
| TRAILING-WHITESPACE-IN-GUARDS | — | none, and the absence is the finding. `ruff` holds a Python line's end and reads no Markdown; `check_text._hygiene` asks for a NUL and a final newline and neither is this. The re-runnable reading is `git ls-files -z '*.md' \| xargs -0 grep -lE '[[:space:]]+$'` | **measured** — 0 of 142 tracked Markdown files. The form first recorded here spelled the class `[ \t]`, which in a POSIX bracket expression is the set {space, backslash, `t`} and not a tab: it names 117 of the 142 and would have read as a repair that never landed. The grammar's owner is the bracket expression, and asking it is what the matcher rule requires of a hand-written one |
...
| GUARD-COUNT-IS-POLLUTION | for 12a, the cleanup restoring the filling | the count beside the immutability guard, which with the cleanup reflects that guard alone | **measured** — with the cleanup, removing the type's wrap turns 2 red; without it, 3. The prediction written here was 1 rather than 2, and it was wrong in both numbers: it was made against the hand-proxy this round then replaced, and it assumed one of the two reds was the pollution when in fact both are cases that read the wrap and the pollution was a third |

## Measured 2026-09-14, at the commit carrying this section — the undeclared-directory check

Written into the section above, inside its table, where it broke the table in two and left three
guard rows rendering as ordinary text. A reading is not a row, and a paragraph between rows is not a
paragraph: CommonMark ends a table at the first line that is not one, so the rows after it stopped
being rows. It also sat under a heading that says nothing was measured, which is the defect the same
round had just repaired one section up.

**The undeclared-directory check, measured 2026-09-14.** Its docstring carried this and no longer
does: a docstring has no date. Against `fornax` at `aa93acd`, every skill folder's directories match
its declared resources and the check reports none — the exemption list is empty because it was
measured empty, not assumed. Against the installed snapshot of `tianheng-foundry` `0.1.0`, it
...
| LINE-RE-EXPORTED-UNDECLARED | for 2a, the four readers routed to the type's own module | none by test, and this is the finding's shape: `shell_script` constructs a `Line` itself, so the import stays and both names go on working. A revert reddens nothing. The reading is `grep -rn 'shell_script\.Line' scripts/ --include='*.py'` | **measured** — it names four sites before the repair and none after. What the repair removes is the second name's readers, not the second name; that it can come back without a signal is why the row says none by test rather than naming one |
...
| Finding | Revert this, once its repair lands | Guard | Red on revert |
|---|---|---|---|
| FAILURES-DISCARDED-ON-AN-EARLIER-RETURN | the report of accumulated failures ahead of the early return in `round_chain.check` | a `test_round_chain` case giving a record an unreadable name while the branch cannot be listed, and asserting the name is still reported | **measured** — restoring the early return turns 1 red |
| PLACE-COLLAPSES-TWO-ABSENCES | the distinct absence variants, back to one value in `round_chain.History` | a `test_round_chain` case giving one record a revision nothing resolves and another a revision off the branch, and asserting the two are reported differently | **measured** — collapsing them back to one answer turns 1 red |
| BASE-RESOLVED-BUT-NEVER-READ, PRIOR-FIELD-COMPARES-ONLY-A-BASENAME | the resolved-identity comparison in `round_chain.names_for`, and the whole-path comparison in `round_chain.prior_field` | `test_round_chain.ChainTests` with a twice-read round spelled once in full and once abbreviated, and a field naming the same last segment under another directory | **measured** — restoring the textual identity comparison turns 1 red, and restoring the last-segment comparison turns 3 |
...
| PROSE-DOES-NOT-DESCRIBE-THE-CHECK | — | none by test while the rule is prose. 1b landed, so the guard is the generated block's own `--check` and the revert is any change to a constant the clauses are written from | **measured** — renaming `round_chain.SECOND_READING` reports the block stale and fails the gate step |
