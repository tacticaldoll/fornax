...
| MATCHER-ENUMERATION-BLIND-TO-NON-REGEX | none — the unit is the `verification` field of `development-knowns.yaml` `shell-command-structure-read-by-hand`, and no check reads a registry entry's prose for whether the command it offers finds what it claims. The re-runnable reading is to run that field's own command and compare what it returns against the grammar decisions the tree actually makes. It missed the one in `read_whole.shell_words`, expressed as a comparison on a scanned token rather than as a compiled pattern; the repair widens the search to the string methods that can bound a token, and running it now surfaces that site. What it cannot decide is which hits bound a borrowed grammar and which merely read a string, which is a reader's judgement and is why the field names a search rather than claiming a list. Repair 3b would make this a guard and was declined once already, inside the same entry. The finding returned after that repair landed in full: the widened search still returns no line from the decisions it was widened for, which is why the record now states the cause as the instrument rather than as the width of its list |
